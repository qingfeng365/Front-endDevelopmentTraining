...
gulp.task('default', ['watch', 'build:style', 'build:jade', 'build:ts', 'copyfile']);
```
...
```js
  ...
  "devDependencies": {
    "typed-we-app": "^1.6.0-update.3"
  }
```

## we-app.d.ts

`/src/typings/we-app.d.ts`
...
/**注册一个自定义组件 */
declare function Component(param: WeApp.ComponentParam);

declare namespace WeApp {
    /**指定自定义组件的属性定义、内部数据和自定义方法 */
    interface ComponentParam {
        /**组件的属性定义,注意：在 properties 定义段中，属性名采用驼峰写法（propertyName）；在 wxml 中，指定属性值时则对应使用连字符写法（component-tag-name property-name="attr value"），应用于数据绑定时采用驼峰写法（attr="{{propertyName}}"） */
        properties?: Object;
        /**组件的初始数据 */
        data?: Object;
        /**组件的自定义方法 */
...
上面文件是 在 `/node_modules/typed-we-app/we-app.d.ts` 的
基础上, 增加 下面 部分代码:
...
## typescript 使用 async/await 的解决方案

需要使用以下支持文件:

`/src/lib/regenerator-runtime.js`

代码示例:

```ts
import regenerator = require('../lib/regenerator-runtime.js');
let regeneratorRuntime = regenerator;
